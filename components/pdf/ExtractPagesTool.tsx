"use client";

import { useState, useEffect } from 'react';
import FileDropzone from '@/components/upload/FileDropzone';
import PdfPageGrid from '@/components/pdf/PdfPageGrid';
import { runInWorker } from '@/lib/worker/client';
import type { UploadedFile } from '@/types/upload';
import type { ProcessingResult } from '@/types/pdf';
import { trackFileSelected, trackProcessingStarted, trackProcessingCompleted, trackProcessingFailed, trackDownloadClicked } from '@/lib/analytics';
import DownloadButton from '@/components/download/DownloadButton';
import ProcessingIndicator from '@/components/ui/ProcessingIndicator';
import ErrorMessage from '@/components/ui/ErrorMessage';

export default function ExtractPagesTool() {
  const [file, setFile] = useState<UploadedFile | null>(null);
  const [selectedPages, setSelectedPages] = useState<number[]>([]);
  const [totalPages, setTotalPages] = useState(0);
  
  const [isProcessing, setIsProcessing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<Blob | null>(null);

  useEffect(() => {
    trackFileSelected('extract_pages');
  }, []);

  const handleFileAccepted = (files: File[]) => {
    if (files.length > 0) {
      const f = files[0];
      setFile({ id: crypto.randomUUID(), file: f, name: f.name, size: f.size });
      setSelectedPages([]);
      setTotalPages(0);
      setResult(null);
      setError(null);
      trackFileSelected('extract_pages');
    }
  };

  const togglePage = (pageIndex: number) => {
    setSelectedPages(prev => 
      prev.includes(pageIndex) 
        ? prev.filter(p => p !== pageIndex) 
        : [...prev, pageIndex]
    );
  };

  const selectAll = () => {
    const all = Array.from({ length: totalPages }, (_, i) => i);
    setSelectedPages(all);
  };

  const clearSelection = () => {
    setSelectedPages([]);
  };

  const handleProcess = async () => {
    if (!file) return;

    if (selectedPages.length === 0) {
      setError("Please select at least one page to extract.");
      return;
    }

    setIsProcessing(true);
    setError(null);
    trackProcessingStarted('extract_pages');

    const startTime = performance.now();

    try {
      // Sort to preserve original order and map to 1-indexed for worker
      const pagesToKeep = [...selectedPages].sort((a, b) => a - b).map(p => p + 1);
      const rangesStr = pagesToKeep.join(',');

      const response = await runInWorker<ProcessingResult<Blob>>('splitPdf', { 
        file: file.file,
        options: { mode: 'extract', rangesStr } 
      });

      if (response.success && response.data) {
        setResult(response.data);
        trackProcessingCompleted('extract_pages', performance.now() - startTime);
      } else {
        throw new Error(response.error?.message || 'Processing failed');
      }
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Unknown error';
      setError(msg);
      trackProcessingFailed('extract_pages', msg);
    } finally {
      setIsProcessing(false);
    }
  };

  const reset = () => {
    setFile(null);
    setResult(null);
    setSelectedPages([]);
    setError(null);
  };

  return (
    <div className="space-y-8">
      {!file && !result && (
        <FileDropzone 
          accept={['application/pdf']}
          multiple={false}
          maxSizeMB={50}
          maxFiles={1}
          onFilesSelected={handleFileAccepted}
        />
      )}

      {file && !result && !isProcessing && (
        <div className="bg-background border border-border rounded-xl p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
            <div>
              <h3 className="font-medium text-foreground">{file.name}</h3>
              <p className="text-sm text-muted">
                Selected: {selectedPages.length} {selectedPages.length === 1 ? 'page' : 'pages'}
              </p>
            </div>
            
            <div className="flex items-center gap-2">
              <button
                onClick={selectAll}
                className="text-sm px-3 py-1.5 rounded bg-muted/20 hover:bg-muted/30 text-foreground transition-colors"
              >
                Select All
              </button>
              <button
                onClick={clearSelection}
                className="text-sm px-3 py-1.5 rounded bg-muted/20 hover:bg-muted/30 text-foreground transition-colors"
              >
                Clear
              </button>
              <button 
                onClick={reset}
                className="text-sm px-3 py-1.5 rounded bg-error/10 hover:bg-error/20 text-error transition-colors ml-2"
              >
                Cancel
              </button>
            </div>
          </div>

          <div className="mb-6 bg-muted/20 rounded-xl p-4 max-h-[60vh] overflow-y-auto border border-border/50">
            <PdfPageGrid 
              file={file}
              selectedPages={selectedPages}
              onTogglePage={togglePage}
              onLoadComplete={setTotalPages}
              selectionMode="extract"
            />
          </div>

          {error && (
            <div className="mb-6">
              <ErrorMessage error={error} />
            </div>
          )}

          <div className="flex justify-end">
            <button
              onClick={handleProcess}
              disabled={isProcessing || selectedPages.length === 0}
              className="rounded-lg bg-primary px-8 py-3 text-sm font-medium text-white hover:bg-primary-hover transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center min-w-[200px]"
            >
              Extract Pages
            </button>
          </div>
        </div>
      )}

      {isProcessing && <ProcessingIndicator message="Extracting pages..." />}

      {result && (
        <DownloadButton 
          blob={result} 
          filename={`extracted-${file?.name}`} 
          onReset={reset}
          onDownload={() => trackDownloadClicked('extract_pages')}
        />
      )}
    </div>
  );
}
