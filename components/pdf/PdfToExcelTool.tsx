...
export default function PdfToExcelTool() {
  ...
  const handleFileAccepted = (files: File[]) => {
    if (files.length > 0) {
      ...
      setResult(null);
      setError(null);
      trackFileSelected('pdf_to_excel');
    }
  };
  ...
  const handleProcess = async () => {
    ...
    setError(null);
    trackProcessingStarted('pdf_to_excel');

    const startTime = performance.now();

    try {
      const response = await runInWorker<ProcessingResult<Blob>>('pdfToExcel', { 
        file: file.file 
      });

      if (response.success && response.data) {
        setResult(response.data);
        trackProcessingCompleted('pdf_to_excel', performance.now() - startTime);
      } else {
      ...
      setError(msg);
      trackProcessingFailed('pdf_to_excel', msg);
    } finally {
  ...
  return (
    <div className="space-y-8">
      ...
      {isProcessing && <ProcessingIndicator message="Analyzing PDF table structure..." />}
      ...
      {result && file && (
        <DownloadButton 
          ...
          onReset={reset}
          onDownload={() => trackDownloadClicked('pdf_to_excel')}
        />
      )}
    </div>
  );
