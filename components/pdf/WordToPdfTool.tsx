...
export default function WordToPdfTool() {
  ...
  const handleProcess = async () => {
    ...
    setError(null);
    trackProcessingStarted('word_to_pdf');

    const startTime = performance.now();

    try {
      const response = await runInWorker<ProcessingResult<Blob>>('wordToPdf', { 
        file: file.file 
      });

      if (response.success && response.data) {
        setResult(response.data);
        trackProcessingCompleted('word_to_pdf', performance.now() - startTime);
      } else {
      ...
      setError(msg);
      trackProcessingFailed('word_to_pdf', msg);
    } finally {
  ...
  return (
    <div className="space-y-8">
      ...
      {isProcessing && <ProcessingIndicator message="Converting Word to PDF..." />}
      ...
      {result && file && (
        <DownloadButton 
          ...
          onReset={reset}
          onDownload={() => trackDownloadClicked('word_to_pdf')}
        />
      )}
    </div>
  );
